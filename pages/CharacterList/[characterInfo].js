...
export default function CharacterInfo(){
  ...
  const BreakthroughButton = ({number, button_brth, breakthrough, setBreakthrough}) => {
    if(button_brth == breakthrough){
      return(
        <Button variant="info" onClick={() => setBreakthrough(button_brth)}>{number}</Button>
      )
    }else{
      return(
        <Button onClick={() => setBreakthrough(button_brth)}>{number}</Button>
      )
    }
  }
  ...
  const BreakthroughButtonGroup = ({rarity}) => {
    ...
    return (
      <ButtonGroup className="character-info-quick-view-breakthrough-col-button">
        {range.map((num) => (
          <BreakthroughButton key={num} number={num} button_brth={'br_'+num} 
